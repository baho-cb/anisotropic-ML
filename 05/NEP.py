...
def calculate_cutoff(pos):
    centraal = np.average(pos,axis=1)
    r_ij = pos - centraal[:,np.newaxis,:]
    rij_norm = np.linalg.norm(r_ij,axis=-1)
    max_val = np.max(rij_norm)
    max_val = np.ceil(max_val * 100) / 100
    return max_val
...
class NEP():
    ...
    def set_pts_types(self,hparams):
        if('pts_types' in hparams):
            self.types = hparams['pts_types']
            self.types = np.array(self.types)
            self.types += 1 
            self.types = cp.asarray(self.types)
            self.types = self.types.astype(cp.float32)    
        else:
            self.types = cp.ones(self.npts*2,dtype=cp.float32)

        if(cp.max(self.types)>4):
            self.types = cp.sqrt(self.types)        
